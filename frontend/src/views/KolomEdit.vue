      ...
      <div class="flex-1 flex flex-col bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div ref="canvasWrapperRef" class="flex-1 overflow-hidden relative bg-gray-100"
          ...
          <div class="absolute origin-top-left select-none"
            ...
            <img v-if="imageSrc" ref="imgRef" :src="imageSrc" class="block max-w-none" draggable="false"
            ...
            <div v-if="seleksi && imageSrc" class="absolute pointer-events-none"
              ...
              <div class="absolute -top-5 left-0 px-1.5 py-0.5 text-white text-xs font-semibold rounded" :style="{ backgroundColor: warnaHex }">{{ namaKolom || 'Kolom' }}</div>
            </div>
...
onMounted(() => {
  ...
  try {
    ...
    if (mode.value === 'tambah') {
      ...
      const list    = data.kolomList || []
      if (editKolomIdx < 0 || editKolomIdx >= list.length) { router.replace(routeKembali.value); return }
      editKolomData = list[editKolomIdx]
    } else {
...
function handleKembali() {
  if (mode.value === 'tambah') {
    ...
    if (raw) { try { const d = JSON.parse(raw); delete d.editKolomIdx; sessionStorage.setItem(SS_TAMBAH, JSON.stringify(d)) } catch {} }
  }
