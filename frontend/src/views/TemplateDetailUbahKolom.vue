    ...
    <div class="flex gap-5" style="height: calc(100vh - 8rem);">
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5 flex flex-col gap-4 overflow-y-auto flex-1">
          ...
          <div>
            ...
            <div class="flex items-center gap-2">
              ...
              <select v-model="halamanDipilih"
                class="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm text-gray-700 bg-white focus:outline-none focus:ring-2 focus:ring-gray-400">
          ...
          <div>
            ...
            <p class="text-xs text-gray-400 mt-1.5">Buat seleksi baru untuk mengubah koordinat.</p>
          </div>
          ...
          <div>
            ...
            <div class="grid grid-cols-2 gap-2">
              <div>
                ...
                <div class="px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm text-gray-600 font-mono select-none">{{ resolusi.width ?? '—' }}</div>
              </div>
              <div>
                <label class="text-xs text-gray-400 mb-1 block">Tinggi (px)</label>
                <div class="px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm text-gray-600 font-mono select-none">{{ resolusi.height ?? '—' }}</div>
              </div>
            </div>
          </div>
          ...
          <button @click="simpanPerubahan" :disabled="!bisaSimpan || isSimpan"
      ...
      <div class="flex-1 flex flex-col bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <div class="flex items-center gap-1 px-4 py-2.5 border-b border-gray-200 bg-gray-50 flex-shrink-0">
          <button @click="activeTool = 'box'" title="Buat Kotak"
          ...
          <button @click="activeTool = 'hand'" title="Geser"
            class="w-8 h-8 flex items-center justify-center rounded-md transition"
          ...
          <button @click="zoomIn" title="Zoom In" class="w-8 h-8 flex items-center justify-center rounded-md text-gray-500 hover:bg-gray-200 transition">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <button @click="zoomOut" title="Zoom Out" class="w-8 h-8 flex items-center justify-center rounded-md text-gray-500 hover:bg-gray-200 transition">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div ref="canvasWrapperRef" class="flex-1 overflow-hidden relative bg-gray-100"
          ...
          <div class="absolute origin-top-left select-none"
            ...
            <div v-if="seleksi && imageSrc" class="absolute pointer-events-none"
              ...
              <div class="absolute -top-5 left-0 px-1.5 py-0.5 text-white text-xs font-semibold rounded" :style="{ backgroundColor: warnaHex }">
                {{ namaKolom || 'Kolom' }}
              </div>
            </div>
...
onMounted(() => {
  ...
  try {
    ...
    editKolom    = data.editKolom
    templateData = data.templateData
    if (!editKolom) { router.replace(`/template/detail/${templateId.value}/ubah`); return }

    namaKolom.value      = editKolom.nama_kolom || ''
    halamanDipilih.value = editKolom.halaman    || 1
    warnaAktif.value     = editKolom.type       || 'green'
    // Isi koordinat dari data kolom yang ada
    koordinat.value = {
      x1: editKolom.x1 ?? null,
      y1: editKolom.y1 ?? null,
      x2: editKolom.x2 ?? null,
      y2: editKolom.y2 ?? null,
    }
    resolusi.value = {
      width:  templateData?.resolusi_width  ?? null,
      height: templateData?.resolusi_height ?? null,
    }
    totalHalaman.value = templateData?.jml_halaman || 1
    muatGambar(editKolom.halaman || 1)
  } catch {
...
function muatGambar(halaman) {
  if (templateData?.path_template_pdf) {
    ...
    const imgPath  = `storage/template/images/${baseName}/${baseName}_page_${halaman}.png`
    imageSrc.value = `${BASE_URL}/${imgPath}`
  }
  // Tampilkan kotak seleksi dari koordinat yang ada
  if (koordinat.value.x1 !== null) {
    seleksi.value = {
      x: koordinat.value.x1,
      y: koordinat.value.y1,
      w: koordinat.value.x2 - koordinat.value.x1,
      h: koordinat.value.y2 - koordinat.value.y1,
    }
  } else {
    seleksi.value = null
  }
...
watch(halamanDipilih, (val) => {
  // Hanya reset koordinat jika pindah ke halaman berbeda
  if (editKolom && editKolom.halaman !== val) {
    seleksi.value   = null
...
function onWheel(e) {
  zoomLevel.value = Math.min(Math.max(zoomLevel.value + (e.deltaY > 0 ? -0.1 : 0.1), 0.2), 5)
}
...
function onMouseDown(e) {
  ...
  if (activeTool.value === 'hand') {
    isPanning.value = true; panStart.value = { x: e.clientX, y: e.clientY }; panOrigin.value = { ...panOffset.value }; return
  }
  if (activeTool.value === 'box' && imageSrc.value) {
    drawing.value = true; const { x, y } = getImageCoords(e); drawStart.value = { x, y }; dragBox.value = { x, y, w: 0, h: 0 }
  }
}
function onMouseMove(e) {
  if (activeTool.value === 'hand' && isPanning.value) {
    panOffset.value = { x: panOrigin.value.x + (e.clientX - panStart.value.x), y: panOrigin.value.y + (e.clientY - panStart.value.y) }; return
  }
  if (activeTool.value === 'box' && drawing.value) {
    const { x, y } = getImageCoords(e)
    dragBox.value = { x: Math.min(x, drawStart.value.x), y: Math.min(y, drawStart.value.y), w: Math.abs(x - drawStart.value.x), h: Math.abs(y - drawStart.value.y) }
  }
}
...
async function simpanPerubahan() {
  ...
  try {
    ...
    alert(err.response?.data?.detail || 'Gagal menyimpan perubahan.')
  } finally {
