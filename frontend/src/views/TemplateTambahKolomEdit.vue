      ...
      <div class="w-72 flex-shrink-0 flex flex-col gap-4">
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5 flex flex-col gap-4 overflow-y-auto flex-1">
          ...
          <div>
            ...
            <p class="text-xs text-gray-400 mt-1.5">Buat seleksi baru pada gambar untuk mengubah koordinat.</p>
          </div>
      ...
      <div class="flex-1 flex flex-col bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div class="flex items-center gap-1 px-4 py-2.5 border-b border-gray-200 bg-gray-50 flex-shrink-0">
          ...
          <div class="flex items-center gap-1">
            <button v-for="w in warnaOptions" :key="w.value"
              @click="warnaAktif = w.value; updateSeleksiWarna()" :title="w.label"
        ...
        >
          ...
          >
            ...
            <!-- Gambar PDF -->
            <img v-if="imageSrc" ref="imgRef" :src="imageSrc"
              class="block max-w-none" draggable="false" @load="onImageLoad" />

            <!-- Kotak seleksi yang sudah ada (dari data kolom) -->
            <div
...
// Data kolom yang sedang diedit
let editKolomIdx  = -1
let editKolomData = null   // data asli kolom sebelum diedit
...
// ── Mount: ambil data kolom dari sessionStorage ───────────────────────
onMounted(() => {
  ...
  try {
    ...
    // Ambil kolom yang akan diedit berdasarkan index
    editKolomIdx  = data.editKolomIdx ?? -1
    const kolomList = data.kolomList || []

    if (editKolomIdx < 0 || editKolomIdx >= kolomList.length) {
      router.replace('/template/tambah')
      return
    }

    editKolomData = kolomList[editKolomIdx]
    ...
    warnaAktif.value     = editKolomData.warna      || 'green'

    koordinat.value = {
      x1: editKolomData.x1 ?? null,
      y1: editKolomData.y1 ?? null,
      x2: editKolomData.x2 ?? null,
      y2: editKolomData.y2 ?? null,
    }

    // Muat gambar halaman yang sesuai
    muatGambar(editKolomData.halaman || 1)
...
function muatGambar(halaman) {
  ...
  // Tampilkan kotak seleksi dari data yang sudah ada
...
watch(halamanDipilih, (val) => {
  // Saat ganti halaman, hapus seleksi dan koordinat
  seleksi.value   = null
...
// ── Update warna seleksi saat warna berubah ───────────────────────────
function updateSeleksiWarna() {
  // Tidak perlu mengubah seleksi, warnaHex adalah computed reactive
}
...
function onMouseUp(e) {
  ...
  if (activeTool.value === 'box' && drawing.value) {
    ...
    // Update koordinat dan tampilkan kotak baru (menggantikan yang lama)
...
function handleKembali() {
  // Bersihkan editKolomIdx agar tidak tersisa di sessionStorage
  const raw = sessionStorage.getItem(SS_KEY)
  if (raw) {
    try {
      const data = JSON.parse(raw)
      delete data.editKolomIdx
...
async function simpanPerubahan() {
  ...
  try {
    ...
    const kolomId = editKolomData?.kolom_id

    // Update kolom di database jika sudah ada id-nya
    if (kolomId) {
    ...
    // Update data kolom di sessionStorage
    const raw = sessionStorage.getItem(SS_KEY)
    if (raw) {
      const data = JSON.parse(raw)
      const kolomList = data.kolomList || []

      // Update kolom pada index yang diedit
      kolomList[editKolomIdx] = {
        ...editKolomData,
        nama_kolom:     namaKolom.value.trim(),
        halaman:        halamanDipilih.value,
        x1:             koordinat.value.x1,
